...
public class SystemTrayApp {
    ...
    public static void main(String[] args) {
        ...
        openItem.addActionListener(e -> {
            LOGGER.info("Popup 'Open' selected (thread={})", Thread.currentThread().getName());
        ...
        exitItem.addActionListener(e -> {
            LOGGER.info("Popup 'Exit' selected (thread={})", Thread.currentThread().getName());
        ...
        trayIcon.addActionListener(e -> {
            LOGGER.info("ActionEvent received from tray icon (thread={})", Thread.currentThread().getName());
        ...
        //Fallback in case the environment delivers MouseEvents (use mouseReleased for compatibility)
        trayIcon.addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) {
                LOGGER.info("Mouse pressed on tray icon, button={} (thread={})", e.getButton(), Thread.currentThread().getName());
                System.out.println("Mouse pressed on tray icon, button=" + e.getButton());
            }

            @Override
            public void mouseClicked(MouseEvent e) {
                LOGGER.info("Mouse clicked on tray icon, clickCount={} button={} (thread={})", e.getClickCount(), e.getButton(), Thread.currentThread().getName());
                System.out.println("Mouse clicked on tray icon, clickCount=" + e.getClickCount() + " button=" + e.getButton());
            }

            @Override
            public void mouseReleased(MouseEvent e) {
                LOGGER.info("Mouse released from tray icon, button={} (thread={})", e.getButton(), Thread.currentThread().getName());
                System.out.println("MouseEvent received from tray icon, button=" + e.getButton());
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "No se pudo agregar el icono al tray: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
